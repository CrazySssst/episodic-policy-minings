...
def load_state(fname):
    scop_name = (tf.get_variable_scope().name + '/ppo' ).lstrip('/')
    saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scop_name))
...
# only load parameters realted to the scope_name
def load_part(fname, scope_name):
    saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope_name))
    saver.restore(tf.get_default_session(), fname)

# only load parameters realted to the scope_name
def save_part(fname, scope_name):
    saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope_name))
    saver.save(tf.get_default_session(), fname)

def load_variables(fname, variables_list):
